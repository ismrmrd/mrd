...
import { getD3ScaffoldHtml } from './d3Scaffold';
...
export function activate(context: vscode.ExtensionContext) {
	...
	const openFileDisposable = vscode.commands.registerCommand('mrd-viz.openFile', async (resource?: vscode.Uri, selectedResources?: vscode.Uri[]) => {
	...
	const compareFilesDisposable = vscode.commands.registerCommand('mrd-viz.compareFiles', async (resource?: vscode.Uri, selectedResources?: vscode.Uri[]) => {
		const targetUris = await resolveCompareTargetUris(resource, selectedResources);
		if (targetUris.length === 0) {
			void vscode.window.showInformationMessage('Select one or more .mrd files to start the D3 comparison scaffold.');
			return;
		}

		const panel = vscode.window.createWebviewPanel(
			'mrd-viz.compareScaffold',
			'MRD Viz Compare (D3 scaffolding)',
			{ viewColumn: vscode.ViewColumn.Active, preserveFocus: false },
			{ enableScripts: true },
		);
		panel.webview.html = getD3ScaffoldHtml(panel.webview, targetUris);
	});

	context.subscriptions.push(openFileDisposable, compareFilesDisposable);
}
...
async function resolveCompareTargetUris(resource?: vscode.Uri, selectedResources?: vscode.Uri[]): Promise<vscode.Uri[]> {
	if (resource?.scheme === 'file' && isMrdFile(resource.fsPath)) {
		return [resource];
	}

	const selectedMrdFiles = selectedResources?.filter(item => item.scheme === 'file' && isMrdFile(item.fsPath)) ?? [];
	if (selectedMrdFiles.length > 0) {
		return selectedMrdFiles;
	}
	...
	const pickedFiles = await vscode.window.showOpenDialog({
		...
		openLabel: 'Compare MRD Files',
	});

	return pickedFiles?.filter(item => item.scheme === 'file' && isMrdFile(item.fsPath)) ?? [];
}
