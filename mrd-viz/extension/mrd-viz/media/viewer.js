  ...
  (function() {
    ...
    function commonSliceDims(tiles) {
      for (let i = 0; i < tiles.length; i++) {
        ...
        if (Array.isArray(dims) && dims.some(function(dim) {
          return (Number(dim.size) || 1) > 1;
        })) {
          return dims;
        }
      }
    ...
    function loadSelectedImage() {
      const canLoad = Number.isInteger(selectedImageIndex) && selectedImageIndex >= 0 && selectedTileThumb && Boolean(selectedTileThumb.renderable);
      if (!canLoad) {
    ...
    function persistState() {
      vscode.setState({ viewportHeight, maximized: isMaximized() });
    }
    ...
    function createImageViewport(tile) {
      ...
      activeViewport = { refit, zoomIn, zoomOut, fit, syncControls: syncMaximizeButton };
