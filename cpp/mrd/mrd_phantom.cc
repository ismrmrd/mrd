...
xt::xtensor<std::complex<float>, 4> generate_noise(std::array<size_t, 4> shape, float sigma, float mean = 0.0) {
  ...
  std::generate(noise.begin(), noise.end(), [&d, &gen]() { return std::complex<float>(d(gen), d(gen)); });
  return noise;
}
...
int main(int argc, char** argv) {
  ...
  Header h;
  h.subject_information.subject_name = "John Doe";
  h.subject_information.subject_id = "1234BGVF";

  EncodingSpace e;
  e.encoded.matrix_size = {1, matrix, 2 * matrix};
  e.encoded.field_of_view = {slice_thickness, fov, 2 * fov};
  e.recon.matrix_size = {1, matrix, matrix};
  e.recon.field_of_view = {slice_thickness, fov, fov};
