...
namespace mrd {
namespace {
void KspaceProtocolWriterBaseInvalidState(uint8_t attempted, [[maybe_unused]] bool end, uint8_t current) {
  std::string expected_method;
  switch (current) {
  case 0: expected_method = "WriteHeader()"; break;
  case 1: expected_method = "WriteKspace() or EndKspace()"; break;
  }
  std::string attempted_method;
  switch (attempted) {
  case 0: attempted_method = "WriteHeader()"; break;
  case 1: attempted_method = end ? "EndKspace()" : "WriteKspace()"; break;
...
void KspaceProtocolReaderBaseInvalidState(uint8_t attempted, uint8_t current) {
  auto f = [](uint8_t i) -> std::string {
    switch (i/2) {
    case 0: return "ReadHeader()";
    case 1: return "ReadKspace()";
    case 2: return "Close()";
...
void KspaceProtocolWriterBase::WriteKspace(mrd::Kspace const& value) {
  if (unlikely(state_ != 1)) {
    KspaceProtocolWriterBaseInvalidState(1, false, state_);
  }

  WriteKspaceImpl(value);
}

void KspaceProtocolWriterBase::WriteKspace(std::vector<mrd::Kspace> const& values) {
  if (unlikely(state_ != 1)) {
    KspaceProtocolWriterBaseInvalidState(1, false, state_);
  }

  WriteKspaceImpl(values);
}

void KspaceProtocolWriterBase::EndKspace() {
  if (unlikely(state_ != 1)) {
    KspaceProtocolWriterBaseInvalidState(1, true, state_);
  }
...
void KspaceProtocolWriterBase::WriteKspaceImpl(std::vector<mrd::Kspace> const& values) {
  for (auto const& v : values) {
    WriteKspaceImpl(v);
  }
}

void KspaceProtocolWriterBase::Close() {
  if (unlikely(state_ != 2)) {
    KspaceProtocolWriterBaseInvalidState(2, false, state_);
  }

  CloseImpl();
}

std::string KspaceProtocolReaderBase::schema_ = KspaceProtocolWriterBase::schema_;

std::vector<std::string> KspaceProtocolReaderBase::previous_schemas_ = KspaceProtocolWriterBase::previous_schemas_;

Version KspaceProtocolReaderBase::VersionFromSchema(std::string const& schema) {
  if (schema == KspaceProtocolWriterBase::schema_) {
    return Version::Current;
  }
  throw std::runtime_error("The schema does not match any version supported by protocol KspaceProtocol.");
}
void KspaceProtocolReaderBase::ReadHeader(std::optional<mrd::Header>& value) {
  if (unlikely(state_ != 0)) {
    KspaceProtocolReaderBaseInvalidState(0, state_);
  }
...
bool KspaceProtocolReaderBase::ReadKspace(mrd::Kspace& value) {
  if (unlikely(state_ != 2)) {
    if (state_ == 3) {
      state_ = 4;
      return false;
    }
    KspaceProtocolReaderBaseInvalidState(2, state_);
  }

  bool result = ReadKspaceImpl(value);
  if (!result) {
    state_ = 4;
  }
  return result;
...
bool KspaceProtocolReaderBase::ReadKspace(std::vector<mrd::Kspace>& values) {
...
// fallback implementation
bool KspaceProtocolReaderBase::ReadKspaceImpl(std::vector<mrd::Kspace>& values) {
  size_t i = 0;
  while (true) {
    if (i == values.size()) {
      values.resize(i + 1);
    }
    if (!ReadKspaceImpl(values[i])) {
      values.resize(i);
      return false;
    }
    i++;
...
void KspaceProtocolReaderBase::Close() {
  if (unlikely(state_ != 4)) {
    if (state_ == 3) {
      state_ = 4;
    } else {
      KspaceProtocolReaderBaseInvalidState(4, state_);
    }
  }

  CloseImpl();
}
void KspaceProtocolReaderBase::CopyTo(KspaceProtocolWriterBase& writer, size_t kspace_buffer_size) {
  {
    std::optional<mrd::Header> value;
    ReadHeader(value);
    writer.WriteHeader(value);
  }
  if (kspace_buffer_size > 1) {
    std::vector<mrd::Kspace> values;
    values.reserve(kspace_buffer_size);
    while(ReadKspace(values)) {
      writer.WriteKspace(values);
    }
    writer.EndKspace();
  } else {
    mrd::Kspace value;
    while(ReadKspace(value)) {
      writer.WriteKspace(value);
    }
    writer.EndKspace();
  }
}
