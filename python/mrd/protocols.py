...
class KspaceProtocolWriterBase(abc.ABC):
    ...
    def write_kspace(self, value: collections.abc.Iterable[Kspace]) -> None:
        """Ordinal 1"""

        if self._state & ~1 != 2:
            self._raise_unexpected_state(2)

        self._write_kspace(value)
        self._state = 3
...
class KspaceProtocolReaderBase(abc.ABC):
    ...
    schema = KspaceProtocolWriterBase.schema
    ...
    def read_kspace(self) -> collections.abc.Iterable[Kspace]:
        """Ordinal 1"""

        if self._state != 2:
            self._raise_unexpected_state(2)

        value = self._read_kspace()
        self._state = 3
        return self._wrap_iterable(value, 4)

    def copy_to(self, writer: KspaceProtocolWriterBase) -> None:
        writer.write_header(self.read_header())
        writer.write_kspace(self.read_kspace())
